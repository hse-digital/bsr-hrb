...
export class ExternalWallMaterialsComponent extends BaseComponent implements IHasNextPage, OnInit {
  ...
  canContinue(): boolean {
    ...
    else if (this.applicationService.currenKbiSection!.ExternalWallMaterials!.indexOf('glass') == -1)
      this.applicationService.currenKbiSection?.ExternalWallMaterials?.push('glass');
  ...
  override canAccess(routeSnapshot: ActivatedRouteSnapshot) {
    return !!this.applicationService.currenKbiSection?.TotalNumberStaircases
      && !!this.applicationService.currenKbiSection?.InternalStaircasesAllFloors;
  }
