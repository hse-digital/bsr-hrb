...
@Component({
  ...
  styleUrls: ['./estimated-percentage.component.scss']
})
export class EstimatedPercentageComponent extends BaseComponent implements IHasNextPage, OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.applicationService.currenKbiSection?.ExternalWallMaterialsPercentage || Object.keys(this.applicationService.currenKbiSection!.ExternalWallMaterialsPercentage).length == 0) {
  ...
  initExternalWallMaterialsPercentage() {
    this.applicationService.currenKbiSection!.ExternalWallMaterialsPercentage = {};
    this.applicationService.currenKbiSection?.ExternalWallMaterials?.forEach(material => {
      this.applicationService.currenKbiSection!.ExternalWallMaterialsPercentage![material] = '';
    });
  }
  ...
  mapExternalWallMaterials(){
    ...
    this.applicationService.currenKbiSection?.ExternalWallMaterials?.forEach(x =>
      aux[x] = !!this.applicationService.currenKbiSection!.ExternalWallMaterialsPercentage![x]
        ? this.applicationService.currenKbiSection!.ExternalWallMaterialsPercentage![x]
        : ''
    );
    this.applicationService.currenKbiSection!.ExternalWallMaterialsPercentage = aux;
  }
  ...
  canContinue(): boolean {
    this.initErrors();
    this.applicationService.currenKbiSection?.ExternalWallMaterials?.forEach(x => this.validateInput(x));
  ...
  validateInput(input: string) {
    ...
    let percentage = this.applicationService.currenKbiSection?.ExternalWallMaterialsPercentage![input];
  ...
  validateSumAllPercentageMustBe100() {
    let percentages = Object.values(this.applicationService.currenKbiSection?.ExternalWallMaterialsPercentage!);
    let totalPercentage = percentages.map(x => Number(x)).reduce((previous, current) => previous + current);
    console.log(percentages, totalPercentage);
    console.log(this.applicationService.currenKbiSection?.ExternalWallMaterialsPercentage!);
