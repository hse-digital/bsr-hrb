...
export class PreviousUseBuildingComponent  extends BaseComponent implements IHasNextPage {
  ...
  canContinue(): boolean {
    this.previousUseBuildingHasErrors = !this.applicationService.currenKbiSection?.PreviousUseBuilding || this.applicationService.currenKbiSection?.PreviousUseBuilding.length == 0;
  ...
  override canAccess(routeSnapshot: ActivatedRouteSnapshot) {
    return !!this.applicationService.currenKbiSection?.ChangePrimaryUse && this.applicationService.currenKbiSection?.ChangePrimaryUse === "yes";
  }
